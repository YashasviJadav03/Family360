...
import { Database, AlertTriangle, CheckCircle2, ShieldAlert, RefreshCw, ArrowRight } from 'lucide-react';
    ...
    return (
      <div className="card-dpi p-6 bg-white border border-slate-border">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs text-slate-secondary">Loading data quality metrics...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="card-dpi p-5 bg-white border border-slate-border space-y-4 shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-border pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-text">Data Quality</h3>
            <p className="text-[11px] text-slate-secondary">
              {total_families.toLocaleString('en-IN')} families · {total_members.toLocaleString('en-IN')} members
            </p>
          </div>
        </div>
      ...
      <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-2">
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-500">
          <span>Target: ≥ 85% for automated DBT</span>
          <span className="text-amber-700 font-medium">
            {(total_families * (1 - complete_profiles_pct / 100)).toFixed(0)} need verification
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="p-3 rounded-lg border border-slate-200 bg-white space-y-2">
          ...
          <Link
            ...
            className="text-[10px] text-navy font-semibold hover:text-orange inline-flex items-center gap-0.5"
          >
            <span>Review</span>
